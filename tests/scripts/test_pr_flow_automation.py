...
def _codacy_success_check(**overrides: object) -> dict[str, object]:
    return {
        "name": "Codacy Static Code Analysis",
        "conclusion": "SUCCESS",
        "detailsUrl": "https://app.codacy.com/gh/owner/repo/pull-requests/225",
        "annotationsCount": 0,
    } | overrides


def _deepsource_python_check(**overrides: object) -> dict[str, object]:
    return {
        "name": "DeepSource: Python",
        "conclusion": "FAILURE",
        "detailsUrl": "https://deepsource.io/gh/owner/repo/run/python",
        "headSha": "abc123",
        "description": "Cyclomatic complexity readability refactor advisory",
    } | overrides


def _merge_ready_pr_with_checks(checks: list[dict[str, object]], **overrides: object) -> dict[str, object]:
    return {
        "state": "OPEN",
        "isDraft": False,
        "mergeable": "MERGEABLE",
...
def _build_deepsource_decision(
    monkeypatch,
    ...
    **pr_overrides: object,
    ...
    monkeypatch.setattr(flow, "pr_view", lambda _repo, _pr: _merge_ready_pr_with_checks(checks, **pr_overrides))
    return flow.build_decision("owner/repo", "225", ignore_self=True, review_threads=review_threads)
...
def test_deepsource_python_advisory_only_failure_does_not_block_merge(monkeypatch):
    """Advisory-only DeepSource Python failure is ignored when all safe evidence is present."""
    decision = _build_deepsource_decision(
        monkeypatch,
        [_check("Unit tests", "SUCCESS"), _codacy_success_check(), _deepsource_python_check()],
    )
    taxonomy = cast(dict[str, Any], decision["blocker_taxonomy"])
...
def test_deepsource_python_safety_failure_still_blocks(monkeypatch):
    ...
    for text in (
        "security bypass in current flow",
        "safety fail-open behavior",
        "crash on malformed input",
    ):
        decision = _build_deepsource_decision(
            monkeypatch,
            [_check("Unit tests", "SUCCESS"), _codacy_success_check(), _deepsource_python_check(description=text)],
        )
        ASSERTIONS.assertFalse(decision["can_merge"])
        ASSERTIONS.assertEqual(decision["blockers"][0]["name"], "DeepSource: Python")


def test_deepsource_python_reproducible_regression_still_blocks(monkeypatch):
    """DeepSource reproducible regression/correctness signals remain blocking."""
    for text in (
        "reproducible regression in merge readiness",
        "correctness failure in status classification",
    ):
        decision = _build_deepsource_decision(
            monkeypatch,
            [_check("Unit tests", "SUCCESS"), _codacy_success_check(), _deepsource_python_check(description=text)],
...
def test_deepsource_required_current_head_failing_check_still_blocks(monkeypatch):
    """Required current-head DeepSource Python failure remains a real blocker."""
    decision = _build_deepsource_decision(
        monkeypatch,
        [_check("Unit tests", "SUCCESS"), _codacy_success_check(), _deepsource_python_check(required=True)],
    )
...
def test_deepsource_missing_check_head_fails_closed(monkeypatch):
    """DeepSource Python failure without check-head evidence remains blocking."""
    decision = _build_deepsource_decision(
        monkeypatch,
        [_check("Unit tests", "SUCCESS"), _codacy_success_check(), _deepsource_python_check(headSha="")],
    )
...
def test_deepsource_stale_check_head_fails_closed(monkeypatch):
    """DeepSource Python failure with stale check-head evidence remains blocking."""
    decision = _build_deepsource_decision(
        monkeypatch,
        [_check("Unit tests", "SUCCESS"), _codacy_success_check(), _deepsource_python_check(headSha="old123")],
    )
...
def test_deepsource_advisory_does_not_override_codacy_action_required(monkeypatch):
    """Codacy ACTION_REQUIRED still blocks even with advisory-only DeepSource."""
    decision = _build_deepsource_decision(
        monkeypatch,
        [_check("Unit tests", "SUCCESS"), _codacy_check(), _deepsource_python_check()],
    )

    ASSERTIONS.assertFalse(decision["can_merge"])
    ASSERTIONS.assertEqual(decision["blockers"][0]["name"], "Codacy Static Code Analysis")
    ASSERTIONS.assertEqual(decision["blockers"][1]["name"], "DeepSource: Python")


def test_deepsource_advisory_does_not_override_pending_checks(monkeypatch):
    """Pending checks keep merge readiness blocked despite advisory-only DeepSource."""
    decision = _build_deepsource_decision(
        monkeypatch,
        [_check("Unit tests", "IN_PROGRESS"), _codacy_success_check(), _deepsource_python_check()],
    )

    ASSERTIONS.assertFalse(decision["can_merge"])
    ASSERTIONS.assertEqual(decision["pending"][0]["name"], "Unit tests")
    ASSERTIONS.assertEqual(decision["blockers"][0]["name"], "DeepSource: Python")


def test_deepsource_advisory_does_not_override_active_review_threads(monkeypatch):
    """Active unresolved review threads keep merge readiness blocked."""
    decision = _build_deepsource_decision(
        monkeypatch,
        [_check("Unit tests", "SUCCESS"), _codacy_success_check(), _deepsource_python_check()],
        review_threads=[{"id": "thread-1", "isResolved": False, "isOutdated": False, "path": "a.py", "line": 9}],
...
def test_unknown_provider_or_status_still_blocks_deepsource_advisory(monkeypatch):
    """Unknown providers/statuses fail closed instead of being treated as DeepSource advisory."""
    unknown_provider = _check("Static Analysis", "FAILURE", "https://scanner.example/check")
    unknown_status = _deepsource_python_check(conclusion="UNKNOWN")

    provider_decision = _build_deepsource_decision(
        monkeypatch,
        [_check("Unit tests", "SUCCESS"), _codacy_success_check(), unknown_provider],
    )
    status_decision = _build_deepsource_decision(
        monkeypatch,
        [_check("Unit tests", "SUCCESS"), _codacy_success_check(), unknown_status],
    )

    ASSERTIONS.assertFalse(provider_decision["can_merge"])
    ASSERTIONS.assertEqual(provider_decision["blockers"][0]["name"], "Static Analysis")
    ASSERTIONS.assertFalse(status_decision["can_merge"])
    ASSERTIONS.assertEqual(status_decision["blockers"][0]["name"], "DeepSource: Python")
