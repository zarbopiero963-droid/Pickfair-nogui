...
def test_cashout_single_event_name_from_partita_label():
    # Branch "Partita:"/"Match:" multi-riga di _extract_event_name (critico per
    # il routing del CASHOUT singolo).
    out = _listener()._parse_cashout_signal("cashout\nPartita:\nNapoli vs Juve")
    assert out is not None
    assert out["signal_type"] == "CASHOUT"
    assert out["event_name"] == "Napoli vs Juve"
