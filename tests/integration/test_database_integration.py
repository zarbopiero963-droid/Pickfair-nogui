...
def test_database_cycle_recovery_checkpoint_monotonic_merge_preserves_stronger_truth():
    with tempfile.TemporaryDirectory() as td:
        ...
        assert loaded["bankroll_sync_status"] == "SYNC_SUCCESS"


@pytest.mark.integration
def test_database_cycle_recovery_checkpoint_merge_preserves_sync_success_on_weaker_reentry():
    with tempfile.TemporaryDirectory() as td:
        db = Database(str(Path(td) / "db.sqlite"))
        key = "batch-4b-sync|evt-4b-sync|1|bet-1"
        db.upsert_cycle_recovery_checkpoint(
            key,
            {
                "settlement_correlation_id": "corr-4b-sync",
                "checkpoint_stage": "BANKROLL_SYNC_DONE",
                "next_trade_submission_status": "NOT_ATTEMPTED",
                "bankroll_sync_status": "SYNC_SUCCESS",
                "reason": "sync_ok",
        ...
        loaded = db.get_cycle_recovery_checkpoint(key)
        assert loaded is not None
        assert loaded["checkpoint_stage"] == "BANKROLL_SYNC_DONE"
        assert loaded["next_trade_submission_status"] == "NOT_ATTEMPTED"
        assert loaded["bankroll_sync_status"] == "SYNC_SUCCESS"
