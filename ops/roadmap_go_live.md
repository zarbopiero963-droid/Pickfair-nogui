...
| 2.2 | **Best price su percorso DIRECT** (B6, decomposto, default-OFF). **B6.1 ✅** estrattore PURO `direct_best_price.resolve_direct_best_price(market_book, selection_id, side, master_price, max_deviation_pct)` (file nuovo, **zero forbidden**, niente broker/bus/runtime): best price **difensivo** — per BACK solo `availableToBack`, per LAY solo `availableToLay`, **mai cross-spread** (a differenza del resolver `aggressive_best_price`); tolleranza vs master (oltre → fallback); **fail-closed** su book assente / mercato non-OPEN (anche `marketDefinition.status`) / runner mancante o **non esplicitamente ACTIVE** (status assente ⇒ fallback, severità pari al market status) / lato senza liquidità (size>0) / prezzo non finito o ≤1.0 / **tolleranza malformata** (None/stringa non numerica ⇒ `invalid_tolerance`, non 0.0 silenzioso) / **selection_id frazionario o bool** (no troncamento `55.9`→`55`) → sempre fallback al `master_price`, mai prezzo inventato. Schema malformato (tipi inattesi) ⇒ fallback, mai eccezione. 25 unit test (marker `unit`). **B6.2 ✅** wiring sul DIRECT dietro **flag globale default-OFF** `use_best_price_direct` (letto via `getattr(self.config, "use_best_price_direct", False)`, **niente edit della config class**). Seam in `RuntimeController._on_signal_received` **dopo** il recheck daily-loss e **prima** di `CMD_QUICK_BET`: metodo `_apply_direct_best_price(payload)`. **Dormiente**: flag assente/False ⇒ no-op assoluto (nessun fetch book, nessuna mutazione payload, comportamento byte-identico). Flag ON ⇒ `betfair_service.get_market_book_snapshot(market_id)` (gia' fail-closed) → `resolve_direct_best_price(...)` (B6.1) → override `payload['price']` col best difensivo entro tolleranza, altrimenti master; audit leggero `best_price_source`/`best_price_reason`; tolleranza via `getattr(config, "best_price_max_deviation_pct", 2.0)`. Tutto il body in try/except ⇒ su errore/book assente/sospeso/no-liquidity/oltre-tolleranza lascia il master price, mai un crash sul percorso d'ordine. Il per-signal `use_best_price` (gia' in `safety_layer`/`risk_middleware`, oggi morto) resta follow-up separato sopra questo kill-switch globale. **Follow-up audit** (Greptile P2): `best_price_source`/`best_price_reason` sono nel payload `CMD_QUICK_BET` e nel log strutturato (override→INFO, fallback→DEBUG), ma `trading_engine._normalize_quick_bet` copia solo chiavi note ⇒ non entrano nei record di lifecycle; la propagazione nei record e' deferita all'attivazione (richiede estendere l'allowlist di `core/trading_engine.py`, fuori scope B6.2). 7 unit test. **B6.3 ⬜** TTL/cancel ordini non abbinati (poller, no retry, persist/notify). | B6 |
