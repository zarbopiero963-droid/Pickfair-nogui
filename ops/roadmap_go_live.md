...
| 2.1-B | **Cashout — routing del segnale** `signal_type` CASHOUT/CASHOUT_ALL → green-up sulle posizioni aperte → `REQ_EXECUTE_CASHOUT`; CASHOUT singolo **ristretto alla partita** (chiude tutte le posizioni di quell'evento, come CASHOUT_ALL ma per il solo `event_name`), CASHOUT_ALL = tutte. Richiede: enrich `telegram_listener` (aggiunge `event_name` al segnale cashout, oggi assente), branch in `runtime_controller._on_signal_received` PRIMA del check campi-obbligatori, enumerazione tavole attive (`table_manager.active_tables`) + prezzo corrente (`betfair_service.get_market_book_snapshot`) + `dutching.dynamic_cashout_single`, wiring executor in `headless_main`. Matematica (`pnl_engine`, `dutching`) già pronta. **Deferred da 2.1-A** (Greptile #296): aggiungere il **consumer `CASHOUT_FAILED`** con notifica operatore per AMBIGUOUS/UNMATCHED/ERROR (oggi solo `CASHOUT_SUCCESS` è consumato → fallimenti real-money silenziosi) e **uniformare lo shape** di `CASHOUT_FAILED` a dict anche in `RiskMiddleware` (oggi pubblica una stringa sullo stesso topic) | Senza questo il copy parziale è più pericoloso di nessun copy (B5) |
