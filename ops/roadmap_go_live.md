...
| 2.3 | **Stake mode** (B7, decomposto, default MM). **B7.1 ✅** resolver PURO `stake_mode.resolve_stake(mode, master_stake, fixed_stake, mm_stake, cap)` (file nuovo, **zero forbidden**): sceglie lo stake del follower per modalità — **MM** (Roserpina, attuale) / **MASTER** (stake del master) / **FIXED** (stake fisso config). **Fail-closed**: modo ignoto/mancante o candidato non valido (non finito/≤0/bool) ⇒ fallback a **MM**; MM non valido ⇒ `0.0` (nessun bet). `cap` opzionale (es. max-single MM): MASTER/FIXED **clampati** al cap ⇒ i limiti di sicurezza del follower non vengono mai superati (MM non capato). 15 unit test. **B7.2 ⬜** master side: `master_stake` nel broadcast `telegram_sender.build_master_signal`. **B7.3 ⬜** follower: parse `master_stake` dal segnale + applica il resolver in `RuntimeController` dietro config (default MM = byte-identico a oggi; MASTER capato col max-single di Roserpina). | B7 |
