...
| 2.1-B1 ✅ | **Cashout — router orchestratore (puro)** (design A3 + identità DB **I1**). `cashout_router.py` (file nuovo, **zero file forbidden**, I/O iniettati → testabile senza broker). `CashoutRouter.route(signal)`: carica gli ordini **del bot dal DB** (identità I1: il `customerOrderRef` NON era inviato a Betfair sul piazzamento (⚠️ da #PR-C `place_bet` invia il `customer_ref` come `customerRef` a livello di richiesta — NON come `customerOrderRef` per-istruzione, che resta non inviato; I1 resta comunque su `bet_id`) → si filtra per `bet_id` ∈ record-del-bot; i record DB danno anche `market_id → event_name`) → `list_current_orders` (stato live abbinato) → `cashout_resolver` (solo bot, lato singolo, miste saltate) → per posizione: gate tradabilità **OPEN** (inline, SUSPENDED saltato best-effort), prezzo live al lato di chiusura, **cancel del resting** non abbinato (prima del cashout), pubblica `REQ_EXECUTE_CASHOUT`. `CASHOUT_ALL` = fan-out N; `CASHOUT` singolo = ristretto all'`event_name` (senza event_name → fail-closed, niente). Fail-closed: `list_current_orders` che solleva → `CASHOUT_FAILED`, abort. 11 unit test. | Cuore del routing, zero forbidden (B5) |
...
| 2.1-B2.2 ✅ | **Cashout — DB identità ordini bot (read-only)** (identità **I1**, design owner **X**). `database.py::get_bot_active_orders()` (additivo, **read-only**, deduplicato per `bet_id`): ritorna `{bet_id, market_id, event_name}` per ogni bet **del bot** viva, unendo i due ledger — **SIM** `simulation_bets` (status `EXECUTABLE`/`EXECUTION_COMPLETE`, `event_name` colonna) e **LIVE** tabella `orders` (`_live_bot_orders`): store **autoritativo** del percorso headless, che costruisce `TradingEngine` **senza** `OrderManager` e quindi NON scrive `order_saga` (Codex P1 — con `order_saga` il cashout live sarebbe stato inerte). `betId` dal `response_json` (`_order_bet_id`: chiave `bet_id`/`betId` di `OrderRouter`, **o** `result.instructionReports` del broker `place_bet`, **o** `raw.instructionReports`), `market_id`/`event_name` dal `payload_json` (request valorizzata da `risk_middleware`). **Denylist** di stati (non allowlist, che ometteva casi — Codex P1/P2): SIM esclude solo `CANCELLED` (include `EXECUTABLE`/`EXECUTION_COMPLETE`/`PARTIAL`); LIVE esclude solo `INFLIGHT` pre-piazzamento (include `MATCHED`/`PARTIALLY_MATCHED`/`COMPLETED`/… — l'identità reale è il `betId` estratto). La liveness è ri-verificata dal router contro `list_current_orders` (le righe terminali stale sono filtrate a valle). Il `customerOrderRef` NON era inviato a Betfair → l'identità è il `bet_id` registrato; liveness reale ri-verificata dal router contro `list_current_orders` (bet_id stale innocui, univoci su Betfair). **(⚠️ aggiornato #PR-C: da PR-C `place_bet` invia il `customerRef` a Betfair — de-dup a livello di richiesta, finestra 60s — NON il `customerOrderRef` per-istruzione, che resta non inviato e quindi vuoto sugli ordini remoti (live); l'identità I1 di questa funzione resta comunque ancorata al `bet_id`. Il `reconciliation_engine` legge `customerOrderRef` solo come segnale opzionale, ma la correlazione non dipende da esso.)** Niente fallback su `event_key` (slug di deduplica che romperebbe il match del CASHOUT singolo, Greptile P1). 11 unit test. **Nota sequencing**: le **costanti** `REQ/CMD_EXECUTE_CASHOUT`/`CASHOUT_SUCCESS/FAILED` si spostano in **B2.4** (definite **e** consumate nello stesso PR dal wiring → niente codice morto, hard-verify §9). | Prerequisito I1, additivo read-only (B5) |
...
| 2.2 | **Best price su percorso DIRECT** (B6, decomposto, default-OFF). **B6.1 ✅** estrattore PURO `direct_best_price.resolve_direct_best_price(market_book, selection_id, side, master_price, max_deviation_pct)` (file nuovo, **zero forbidden**, niente broker/bus/runtime): best price **difensivo** — per BACK solo `availableToBack`, per LAY solo `availableToLay`, **mai cross-spread** (a differenza del resolver `aggressive_best_price`); tolleranza vs master (oltre → fallback); **fail-closed** su book assente / mercato non-OPEN (anche `marketDefinition.status`) / runner mancante o **non esplicitamente ACTIVE** (status assente ⇒ fallback, severità pari al market status) / lato senza liquidità (size>0) / prezzo non finito o ≤1.0 / **tolleranza malformata** (None/stringa non numerica ⇒ `invalid_tolerance`, non 0.0 silenzioso) / **selection_id frazionario o bool** (no troncamento `55.9`→`55`) → sempre fallback al `master_price`, mai prezzo inventato. Schema malformato (tipi inattesi) ⇒ fallback, mai eccezione. 25 unit test (marker `unit`). **B6.2 ✅** wiring sul DIRECT dietro **flag globale default-OFF** `use_best_price_direct` (letto via `getattr(self.config, "use_best_price_direct", False)`, **niente edit della config class**). Seam in `RuntimeController._on_signal_received`: metodo `_apply_direct_best_price(payload)` eseguito **PRIMA dei gate finali pre-submit**, che vengono **RIESEGUITI dopo lo snapshot** (Codex P2: il fetch è lento e in LIVE può invalidare la sessione, quindi non deve aggirare i gate). Ordine: snapshot → (a) session-guard LIVE re-check (solo se snapshot tentato) → (b) recheck daily-loss/emergency → solo allora `SIGNAL_APPROVED` e `CMD_QUICK_BET`. Se dopo lo snapshot la sessione è invalida o daily-loss/emergency è pending: niente `SIGNAL_APPROVED`/`CMD_QUICK_BET`, `SIGNAL_REJECTED` (`*:pre_submit_recheck`), risorse rilasciate (`_release_acquired_and_reject`). **Dormiente**: `_apply_direct_best_price` ritorna `False` con flag OFF ⇒ no-op assoluto (nessun fetch, nessuna mutazione payload, nessun gate nuovo, byte-identico a oggi); `True` con flag ON ⇒ il chiamante riesegue i gate. Flag ON ⇒ `betfair_service.get_market_book_snapshot(market_id)` (gia' fail-closed) → `resolve_direct_best_price(...)` (B6.1) → override `payload['price']` col best difensivo entro tolleranza, altrimenti master; audit leggero `best_price_source`/`best_price_reason` (override→INFO, fallback→DEBUG); tolleranza via `getattr(config, "best_price_max_deviation_pct", 2.0)`. Tutto il body in try/except ⇒ mai un crash sul percorso d'ordine. 11 unit test (7 estrattore-in-seam + 4 ordine-gate). **⚠️ B6.2 NON è LIVE-ready**: dormiente fino alla PR di attivazione che risolve i deferred sotto. **Attivazione LIVE (decomposta in 2 PR, flag resta default-OFF)**: **PR-1 ✅ priceProjection** — `betfair_client.get_market_book` e `betfair_service.get_market_book_snapshot` hanno un param opt-in `include_prices` (default `False` = invariato per i chiamanti esistenti); `_apply_direct_best_price` chiama con `include_prices=True` ⇒ `listMarketBook` chiede le ladder `EX_BEST_OFFERS` (senza, in LIVE il book tornava senza ladder e il best-price faceva sempre fallback al master, Codex P2 #2). In SIMULATION il flag è ininfluente (broker già completo). **PR-2 ✅ propagazione audit-record** — `best_price_source`/`best_price_reason` aggiunti a `_PASSTHROUGH_KEYS` (`trading_constants`) e copiati nei record/audit del `TradingEngine` (`_build_terminal_metadata`, reconcile-meta, audit dict) via helper `_copy_best_price_meta`: la provenienza dell'override entra ora nei record di esecuzione e nel result, non solo nei log (Greptile P2 chiuso). **Residuo deferred** (non bloccante): per-signal `use_best_price` (gia' in `safety_layer`/`risk_middleware`, morto) come secondo livello sopra il kill-switch globale. **B6.3** TTL/cancel ordini DIRECT non abbinati (decomposto, no retry). **B6.3.1 ✅** detector PURO `direct_unmatched_ttl.select_expired_unmatched(current_orders, now_epoch, ttl_seconds, direct_bet_ids, scope_market_ids)` (file nuovo, **zero forbidden**): individua gli ordini **DIRECT** (bet_id in `direct_bet_ids`) **completamente non abbinati** (`sizeMatched==0` e `sizeRemaining>0`) la cui età (`now − placedDate`/`placed_epoch`) supera il TTL → lista `{market_id, bet_id}` da cancellare, deduplicata per `bet_id`. **Scope DIRECT obbligatorio** (Greptile P1): in una lista ordini mista, senza allowlist (vuota/assente) **non** seleziona nulla → mai cancellare cashout/green-up/copy. **Fail-closed**: TTL/now non validi (incl. `bool` → no `True`=1.0, e overflow `float()` → no-raise), riga malformata, size **non finite** (NaN/inf), età non determinabile (`placed_epoch` malformato ricade su `placedDate`; entrambi assenti/illeggibili ⇒ skip) ⇒ **non** selezionato, mai cancellare a vuoto. Cancellazione residuo di ordini parzialmente abbinati = non-goal. 23 unit test. **B6.3.2** wiring (decomposto). **B6.3.2a ✅** registry in-memory `customer_ref → bet_id` degli ordini DIRECT in `RuntimeController` (`_track_direct_order`, property `direct_unmatched_bet_ids`): **identità certa dal runtime, niente DB / niente deduzione fragile**. Popolato solo su ack (`QUICK_BET_ACCEPTED`/`PARTIAL`) di un ordine che porta `best_price_source` (marker del percorso best-price DIRECT, già propagato via `_PASSTHROUGH_KEYS` in PR-2) + `bet_id` + `customer_ref`; pulito **solo** quando l'ordine non è più resting vivo (`FILLED`/`FAILED`/`ROLLBACK_DONE`) per `customer_ref` (presente anche in `ROLLBACK_DONE` che non porta il bet_id). `SUCCESS`/`AMBIGUOUS` **non** puliscono: un place riuscito/incerto può lasciare un ordine non abbinato vivo, che deve restare nell'allowlist del poller TTL (Greptile P2). **Fail-safe**: in-memory, vuoto al restart ⇒ mai un bet_id senza identità certa nell'allowlist. Verificato che gli eventi ack/terminali del RuntimeController portano `best_price_source`+`bet_id`+`customer_ref` (catena CMD_QUICK_BET→trading_engine→order_manager preserva via dict-copy), quindi **nessun edit di `trading_engine`**. Nessun poller, nessun cancel, nessun side-effect broker. 14 unit test. **B6.3.2b ✅** poller/cancel reale dietro **flag globale default-OFF** `direct_unmatched_ttl_enabled` (via `getattr`, niente edit config class). `_poll_direct_unmatched_ttl` gated su `direct_unmatched_ttl_poll_sec` (default 30s), cablato nei tick market-data (`_stream_market_book_callback` + `_snapshot_rest_fallback`): `list_current_orders` → `select_expired_unmatched(direct_bet_ids=direct_unmatched_bet_ids, ttl=direct_unmatched_ttl_sec)` → cancel via **`CashoutCancelAdapter`** (B2.1), che normalizza firma e risposta tra **LIVE** (`bet_ids`) e **SIM** (`instructions`) ⇒ niente mismatch di contratto col simulation broker (Greptile P1). Nessun client cancel (transitorio) ⇒ skip senza scartare (ritenta al prossimo giro, non è un fallimento). **Sicurezza**: solo ordini DIRECT noti (allowlist registry B6.3.2a — mai cashout/green-up/copy/ignoti); allowlist vuota ⇒ nessun cancel; **NO RETRY** (un tentativo per ordine, poi `bet_id` esce dal registry, esito su bus `DIRECT_UNMATCHED_CANCELLED`/`_FAILED`); **fail-closed** (errore `list_current_orders` aborta il giro, ogni eccezione catturata, niente crash sul percorso). 12 unit test (incl. mai-cancellare-fuori-registry). **B6 COMPLETO** (resta default-OFF, non abilitato in produzione). | B6 |
| 2.3 | **Stake mode** (B7, decomposto, default MM). **B7.1 ✅** resolver PURO **keyword-only** `stake_mode.resolve_stake(*, mode=None, master_stake=None, fixed_stake=None, mm_stake=None, cap=None)` (file nuovo, **zero forbidden**; i chiamanti B7.2/B7.3 devono passare gli argomenti **per keyword**, mai posizionali): sceglie lo stake del follower per modalità — **MM** (Roserpina, attuale) / **MASTER** (stake del master) / **FIXED** (stake fisso config). **Fail-closed**: modo ignoto/mancante (anche `mode` omesso ⇒ default `None` ⇒ no `TypeError`) o candidato non valido (non finito/≤0/bool) ⇒ fallback a **MM**; MM non valido ⇒ `0.0` (nessun bet). `cap` opzionale (es. max-single MM): `cap is None` = nessun limite; `cap` finito **≥0** = limite reale, MASTER/FIXED **clampati** al cap (`cap==0` ⇒ stake `0.0` = nessun bet, mai fail-open) ⇒ i limiti di sicurezza del follower non vengono mai superati (MM non capato); `cap` malformato (negativo/non finito/non numerico) ⇒ **fail-closed a MM** (`invalid_cap`), mai ignorato. Ritorna `{stake, mode, reason, requested_mode}`. 19 unit test. **B7.2 ⬜** master side: `master_stake` nel broadcast `telegram_sender.build_master_signal`. **B7.3 ⬜** follower: parse `master_stake` dal segnale + applica il resolver (chiamata **keyword-only**) in `RuntimeController` dietro config (default MM = byte-identico a oggi; MASTER capato col max-single di Roserpina). ⚠️ **Prerequisito packaging (Greptile P1)**: `stake_mode.py` è top-level e non coperto dai package glob (`core*`/`services*`/…) ⇒ prima del wiring runtime B7.3 va deciso il suo home installabile (relocate in un package incluso, con Matrix Phase 0) per evitare `ModuleNotFoundError` in deploy. | B7 |
