...
| 1.4 ✅ | **Daily-loss breach → kill switch**: enforcement **SINCRONO** in `_on_close_position` — dopo aver applicato il PnL e **PRIMA del bankroll-sync (rete) e della submission auto-trade**, `_enforce_daily_loss_hard_stop` chiama `emergency_stop` (cancel-all + persist + lockdown, fail-closed) se breached, così l'auto-trade è bloccato da `_risk_allows_auto_trade` e **nessun ordine (auto o segnale concorrente) parte dopo il breach**. Difese: **precheck** a inizio metodo (stato monitor cached) che fail-closa anche i rami di early-return (rejected/recovery-ambiguo); enforce su **ACTIVE o PAUSED** (un settlement in pausa ferma) + **`resume()` rifiuta** se breached same-day; atomico sotto lock (no doppio stop); `emergency_stop` robusto al fallimento dello status-snapshot (cancel-all anche in outage); persist sincrono al rilevamento. | B4 risolto |
