...
| Task | Descrizione | Note |
|---|---|---|
| 2.0 ✅ | **Prep cashout — normalizzazione broker in `OrderRouter`**: `place()` filtra i kwargs per la firma del broker attivo (fix `TypeError` sul client live, che non accetta `customer_ref`/`event_key`/`table_id`/`batch_id`) e normalizza il ritorno a `{ok, status, matched, bet_id, error, raw}` deterministico/fail-closed (sim non abbinato → `ok=False`). È il seam unico consentito dai guardrail `no-bypass` (`.place_bet(` ammesso solo nei layer broker + `core/order_router.py`), quindi l'executor del cashout piazzerà via router. Broker `place_bet` non toccati. | Prerequisito 2.1 (B5); router prima sim-only/rotto per il live |
| 2.1 | **Cashout mirror**, 3 pezzi: publisher `CASHOUT_SUCCESS` all'esecuzione; routing `signal_type` CASHOUT → `REQ_EXECUTE_CASHOUT`; executor per `CMD_EXECUTE_CASHOUT` (piazza via `OrderRouter` normalizzato, 2.0). Bus REQ→CMD e matematica (`pnl_engine`, `dutching`) già pronti. Routing: il segnale cashout porta il nome partita → resolver tavolo-attivo-per-`event_name` (`event_key` = market:selection, non ricostruibile dal nome) → green-up sulla posizione aperta | Senza questo il copy parziale è più pericoloso di nessun copy (B5) |
