...
| 1.2 ✅ | **Keepalive sessione betting**: `BetfairClient.keep_alive()` (chiamata leggera `getAccountFunds`, non muta ordini/conto) + loop daemon ~600s in `BetfairService` (mirror del keepalive streaming), avviato dopo il login live e fermato in `disconnect()`. Su `SESSION_EXPIRED`/`INVALID_SESSION` instrada al re-auth **fail-closed bounded esistente** (`handle_session_expiry`); errore generico = soft-log + contatore (loop continua). Re-entrancy-safe: il re-auth parte dal thread di keepalive → `_stop_session_keepalive` non fa self-join (stop-event per-thread, start idempotente); niente keepalive in simulazione o a sessione invalida. Metriche via `keepalive_status()`. | B2 risolto |
