...
| **Stato cashout (B5)** 📌 | **Indice di avanzamento** (aggiornato a ogni PR): 2.0 ✅ · 2.1-A ✅ · B0 ✅ · B1 ✅ · **B2.1 ✅ · B2.2 ✅ · B2.3 ✅ · B2.4a ✅ · B2.4b-1 ✅** · B2.4b-2 ⬜ · B2.5 ⬜ · 2.1-C ⬜. **PR rimanenti per chiudere B5: 3** — B2.4b-2 (trigger runtime CASHOUT/CASHOUT_ALL, attiva il cashout), B2.5 (residui 2.1-A), 2.1-C (copy-pattern action) — **+ deliverable finale** (spiegazione completa con esempi numerici reali + possibili aggiunte di funzionalità). Nota: i moduli puri (resolver/router/adapter/identità) sono pronti ma **inerti** finché B2.4 non li cabla nel runtime. | Checklist sempre visibile |
...
| 2.1-B2.4b-2 ⬜ | **Cashout — trigger runtime (attiva il cashout)**. Branch in `core/runtime_controller._on_signal_received` per `signal_type ∈ {CASHOUT, CASHOUT_ALL}` **dopo** il gate emergency-stop (E1 gratis) e `_runtime_active`, **prima** del check campi-obbligatori (che scarterebbe `CASHOUT_ALL` come `campi_mancanti`); applica gli stessi gate live (emergency stop, daily-loss pending stop, deploy gate, runtime active, session valid); costruisce il `CashoutRouter` (B1) coi servizi reali + `CashoutCancelAdapter` (B2.1) via `betfair_service.get_live_client`/`get_simulation_broker`; il runtime pubblica **solo** `REQ_EXECUTE_CASHOUT` (mai `CMD_*` diretto). **Costanti** `REQ/CMD_EXECUTE_CASHOUT`/`CASHOUT_SUCCESS/FAILED` in `core/trading_constants.py`. Consumer/notifica `CASHOUT_FAILED`. `UNMATCHED`/`AMBIGUOUS` senza retry automatico (cancel sicuro o persist/reconcile). Forbidden da registrare: `core/runtime_controller.py`, `core/trading_constants.py`. | Accende il cashout end-to-end (B5) |
