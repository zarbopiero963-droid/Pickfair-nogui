...
| 1.4 ✅ | **Daily-loss breach → kill switch**: enforcement **SINCRONO** in `_on_close_position` — dopo aver applicato il PnL del settlement e **PRIMA** della submission auto-trade, `_enforce_daily_loss_hard_stop` chiama `emergency_stop` (cancel-all + persist + lockdown, fail-closed) se breached **e** runtime ACTIVE, così l'auto-trade successivo viene bloccato da `_risk_allows_auto_trade` e **nessun ordine parte dopo il breach**. Atomico sotto lock (no doppio stop); gated su ACTIVE (no stop da status-poll all'avvio); `emergency_stop` reso robusto al fallimento dello status-snapshot (cancel-all garantito anche in outage). Persist sincrono al rilevamento. | B4 risolto |
