...
| 2.5 | **Parser Personalizzato** (epica, portata da `xtrader-bridge`, decomposta pure-first). Estrazione configurabile a delimitatori dei campi-segnale da un messaggio Telegram (senza parser hardcoded); a valle il concetto **Provider per-canale** (mappe nomi/mercati) e il **sync catalogo Betfair** risolvono `MarketId`/`SelectionId` (il placement lo fa già Pickfair). **P1 ✅** core estrazione **PURO** in `services/custom_parser/` (package incluso nei glob `services*` ⇒ shippato, evita il trap packaging Greptile P1; **zero forbidden**): `model` (`FieldRule`/`CustomParserDef` + validazione strutturale, target = 8 campi essenziali `PARSER_TARGETS` invece delle 14 colonne CSV XTrader) · `engine` (`extract_value` a delimitatori **tolleranti agli spazi**, `extract_between`, `extract_scores` #325 con cap difensivo, `apply_parser`→`ExtractionResult`) · `transforms` (`score_to_over`, fail-closed su implausibili) · `value_maps` (built-in `bettype`→**BACK/LAY**; alias ambiguo/monolettera/ignoto ⇒ vuoto ⇒ "Non pronto", mai indovinare il lato). Fail-closed ovunque; dormiente (nessun wiring runtime). 35 unit test. **Rimandati** (PR successive, decomposte): persistenza (JSON vs **SQLite**) · modalità riconoscimento (NAME-in→ID-out) · pipeline→`SegnaleRisolto` · output multi-riga · profili mappatura nomi/mercati · routing per-chat/Provider · **sync catalogo Betfair** (solo calcio, upsert + delete stantii, ID effimeri) · GUI tab Provider su `mini_gui`. | Copy: parser configurabile per-canale |
