...
def can_auto_push(context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if gate["allowed"]:
        return mode_result
    return automation_disabled_result("push", mode_result["mode"], str(gate["reason"]), next_action="needs_manual")


_POST_FIX_AUDIT_GATE_KNOWN_STATUSES = {"PASS", "FAIL", "PARTIAL", "NEEDS_MANUAL", "UNKNOWN"}


def _strict_gate_bool(value: object) -> tuple[bool | None, bool]:
...
def _strict_gate_bool_from_sources(*values: object) -> tuple[bool | None, bool, bool]:
...
def _normalize_post_fix_audit_status_from_context(
    ...
    if len(unique) > 1:
        return "", False, True
    if unique[0] not in _POST_FIX_AUDIT_GATE_KNOWN_STATUSES:
        return "", False, True
    return unique[0], False, False


def normalize_post_fix_audit_gate_input(context: dict[str, Any] | None = None) -> dict[str, Any]:
    ctx = dict(context or {})
    report = ctx.get("post_fix_audit_report") if isinstance(ctx.get("post_fix_audit_report"), dict) else {}
    rollback = ctx.get("rollback") if isinstance(ctx.get("rollback"), dict) else {}
    scope_audit = ctx.get("scope_audit") if isinstance(ctx.get("scope_audit"), dict) else {}
    ...
    next_action = str(
        first_nonempty(
            ...
            (ctx.get("report") if isinstance(ctx.get("report"), dict) else {}).get("next_action"),
            rollback.get("next_action"),
        )
        or ""
    ).strip()
    status = str(first_nonempty(ctx.get("status"), rollback.get("status")) or "").strip().upper()
    rollback_scope_failed = (
        audit == "FAIL"
        and status == "FAIL"
        and next_action == "needs_manual_scope_violation_rollback_failed"
    ...
    if scope_allowed_missing and "allowed" in scope_audit:
        ...
        if isinstance(scope_audit_allowed, bool):
            scope_allowed = scope_audit_allowed
            scope_allowed_missing = False
            scope_allowed_malformed = False
        else:
            scope_allowed = None
            scope_allowed_missing = False
    ...
    if validation_passed_missing:
        missing_fields.append("validation_passed")
    if current_head_matches_missing:
        missing_fields.append("current_head_matches")
    if dirty_worktree_missing:
        missing_fields.append("dirty_worktree")
    if task_no_commit_push_missing:
        missing_fields.append("task_no_commit_push")
    if scope_allowed_missing:
        missing_fields.append("scope_allowed")
    if rollback_attempted_missing:
    ...
    if validation_passed_malformed:
        malformed_fields.append("validation_passed")
    if current_head_matches_malformed:
        malformed_fields.append("current_head_matches")
    if dirty_worktree_malformed:
        malformed_fields.append("dirty_worktree")
    if task_no_commit_push_malformed:
        malformed_fields.append("task_no_commit_push")
    if scope_allowed_malformed:
        malformed_fields.append("scope_allowed")
    if rollback_attempted_malformed:
        malformed_fields.append("rollback_attempted")
    if rollback_succeeded_malformed:
        malformed_fields.append("rollback_succeeded")
    if can_commit_malformed:
    ...
    return {
        ...
        "post_fix_audit_malformed": post_fix_audit_malformed,
        "validation_passed": validation_passed,
        "current_head_matches": current_head_matches,
        "dirty_worktree": dirty_worktree,
        "task_no_commit_push": task_no_commit_push,
...
def build_post_fix_audit_gate_result(
    *,
    allowed: bool,
    can_commit: bool,
    can_push: bool,
    ...
    return {
        ...
        "post_fix_audit": str(post_fix_audit or ""),
        "reason": str(reason or ""),
        "next_action": str(next_action or ""),
        "needs_manual": bool(needs_manual),
        "missing_fields": sorted(set(str(item) for item in (missing_fields or []) if str(item).strip())),
    }
...
def evaluate_post_fix_audit_gate(context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if normalized["scope_allowed"] is False:
        ...
        reason = "rollback_failed"
    elif normalized["post_fix_audit_malformed"]:
        reason = "post_fix_audit_malformed"
    elif normalized["post_fix_audit_missing"]:
    ...
    elif "validation_passed" in missing_fields:
        reason = "validation_missing"
    elif normalized["validation_passed"] is not True:
        reason = "validation_failed"
    elif "current_head_matches" in missing_fields:
    ...
    if not can_commit:
        can_push = False
    allowed = can_commit and can_push and reason == "allowed"
    return build_post_fix_audit_gate_result(
        allowed=allowed,
        can_commit=can_commit,
        can_push=can_push,
        status="PASS" if allowed else "FAIL",
        post_fix_audit=post_fix_audit or "UNKNOWN",
        reason=reason,
        next_action="proceed" if allowed else "needs_manual",
        needs_manual=not allowed,
        missing_fields=missing_fields,
    )


def can_commit_after_post_fix_audit(context: dict[str, Any] | None = None) -> dict[str, Any]:
    result = evaluate_post_fix_audit_gate(context)
    allowed = bool(result["can_commit"])
    return {
        **result,
        "allowed": allowed,
        "status": "PASS" if allowed else "FAIL",
...
def can_push_after_post_fix_audit(context: dict[str, Any] | None = None) -> dict[str, Any]:
    result = evaluate_post_fix_audit_gate(context)
    allowed = bool(result["can_push"])
    return {
        **result,
        "allowed": allowed,
        "status": "PASS" if allowed else "FAIL",
...
def assert_post_fix_audit_gate_or_block(context: dict[str, Any] | None = None) -> dict[str, Any]:
    result = evaluate_post_fix_audit_gate(context)
    if result["allowed"]:
        return result
    if result["reason"] in {"commit_not_allowed", "push_not_allowed", "task_no_commit_push"}:
        raise PermissionError(f"post-fix audit gate denied: {result['reason']}")
    raise RuntimeError(f"post-fix audit gate denied: {result['reason']}")
...
def build_post_fix_audit_retry_task(
    *,
    ...
    scope = str(original_task_scope or "").strip() or "N/A"
    sections = [
        ...
        _format_failure_reason_block(failure_reason),
        f"Original task scope:\n{scope}",
        _render_retry_file_section("Files allowed", _normalized_retry_file_list(files_allowed)),
        _render_retry_file_section("Files forbidden", _normalized_retry_file_list(files_forbidden)),
        _retry_no_commit_push_instructions(),
    ]
...
def _post_fix_retry_options(options: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "ledger_base_dir": str(
            options.get("ledger_base_dir")
            or options.get("automation_ledger_base_dir")
            or options.get("ledger_dir")
            or ""
        ),
        "ledger_pr_number": options.get("pr_number") or options.get("pr") or ledger_metadata.get("pr"),
        "ledger_metadata": ledger_metadata,
    }


def _post_fix_retry_state(report: dict[str, Any] | None, opts: dict[str, Any]) -> dict[str, Any]:
    status, exact_reason, normalized_reason, repeated = _post_fix_report_outcome(
        report, opts.get("previous_failure_reasons")
    )
    raw = str(((report or {}).get("status") or "")).strip().upper() if isinstance(report, dict) else ""
    return {
        "status": status,
        "raw_status": raw,
        ...
        "retry_budget_exhausted": opts["retry_count"] >= opts["retry_limit"],
    }


def _post_fix_retry_block_flags(state: dict[str, Any]) -> dict[str, bool]:
    malformed = not _recognized_post_fix_raw_status(state["raw_status"])
    downgraded_pass = state["raw_status"] in {"PASS", "PASSED"} and state["status"] != "PASS"
...
def _post_fix_retry_ledger_base_dir(options: dict[str, Any]) -> Path | None:
    raw_base = options.get("ledger_base_dir")
    if raw_base in (None, ""):
        return None
    return _normalized_ledger_base_dir(raw_base)


def _post_fix_retry_ledger_target(
    options: dict[str, Any], event: dict[str, Any]
) -> tuple[Path, Path | None]:
    target = Path(str(options.get("ledger_path") or "")).expanduser()
    base = _post_fix_retry_ledger_base_dir(options)
    if base is None:
        _validate_automation_ledger_append_path(target)
        return target, None

    pr_number = options.get("ledger_pr_number")
    if pr_number in (None, ""):
        pr_number = event.get("pr") if isinstance(event, dict) else None

    expected = automation_ledger_path(base, pr_number)
    _validate_automation_ledger_append_path(target, base_dir=base)
    if target.resolve(strict=False) != expected.resolve(strict=False):
        raise ValueError("automation ledger path must match the configured ledger base and PR")
    return expected, base


def _append_post_fix_retry_ledger_event(
    options: dict[str, Any], state: dict[str, Any], decision: dict[str, Any]
) -> None:
    if not options["ledger_path"]:
        return
    event_type = "post_fix_audit_retry_scheduled" if decision["should_retry"] else "post_fix_audit_retry_blocked"
    event = build_automation_ledger_event(
        event_type=event_type,
        next_action=decision["next_action"],
        reason=state["failure_reason"] or state["status"],
        details=_post_fix_retry_ledger_details(options, state, decision),
        **(options["ledger_metadata"] or {}),
    )
    ledger_target, ledger_base = _post_fix_retry_ledger_target(options, event)
    append_automation_ledger_event(ledger_target, event, base_dir=ledger_base)


def _post_fix_retry_ledger_details(
    options: dict[str, Any], state: dict[str, Any], decision: dict[str, Any]
) -> dict[str, Any]:
    return {
        "status": state["status"],
        "malformed_status": decision["malformed_status"],
        "failure_reason": state["failure_reason"],
        "failure_reason_normalized": state["failure_reason_normalized"],
        "repeated_failure": state["repeated_failure"],
        "retry_count": options["retry_count"],
        "retry_limit": options["retry_limit"],
        "retry_budget_exhausted": state["retry_budget_exhausted"],
    }
...
def _build_post_fix_retry_decision(
    state: dict[str, Any], *, should_retry: bool, malformed_status: bool, retry_task: str
) -> dict[str, Any]:
    return {
        "status": state["status"],
        "malformed_status": malformed_status,
        "should_retry": should_retry,
        "next_action": _retry_decision_next_action(state["status"], should_retry),
...
def _post_fix_retry_task(original_task_scope: str, state: dict[str, Any], opts: dict[str, Any]) -> str:
    return build_post_fix_audit_retry_task(
        original_task_scope=original_task_scope,
        failure_reason=state["failure_reason"],
        files_allowed=opts["files_allowed"],
        files_forbidden=opts["files_forbidden"],
    )
...
def decide_post_fix_audit_retry(
    ...
    **options: Any,
    ...
    opts = _post_fix_retry_options(options)
    state = _post_fix_retry_state(report, opts)
    flags = _post_fix_retry_block_flags(state)
    should_retry = _post_fix_retryability(state, flags)
    retry_task = _post_fix_retry_task(original_task_scope, state, opts) if should_retry else ""
    decision = _build_post_fix_retry_decision(
        state, should_retry=should_retry, malformed_status=flags["malformed_status"], retry_task=retry_task
    )
    _append_post_fix_retry_ledger_event(opts, state, decision)
    return decision
...
def _validate_automation_ledger_append_path(path: Path, base_dir: str | Path | None = None) -> None:
    ...
    if base_dir is not None:
        root = _normalized_ledger_base_dir(base_dir)
        _validate_ledger_child_path(root, path.parent)
...
def append_automation_ledger_event(
    path: str | Path,
    event: dict[str, Any],
    *,
...
def read_automation_ledger_events(
    path: str | Path,
    *,
    base_dir: str | Path | None = None,
) -> list[dict[str, Any]]:
...
__all__ = (
    ...
    "post_fix_audit_failure_repeated",
)
