...
class HeadlessApp:
    ...
    def __init__(self):
        ...
        self.dutching_risk_middleware: Optional[RiskMiddleware] = None
        self.dutching_controller: Optional[DutchingController] = None
    ...
    def _reset_runtime_refs(self) -> None:
        ...
        self.runtime = None
        self.dutching_risk_middleware = None
        self.dutching_controller = None
    ...
    def build(self, start_services: bool = True) -> None:
        ...
        try:
            ...
            # NIENTE assegnazione di betfair_client qui: al build la
            # connessione non esiste ancora e congelarlo a None farebbe
            # scavalcare breaker e gestione sessione nel ramo LIVE. Il
            # client si risolve LAZY a ogni submission via client_getter
            # (TradingEngine._resolve_live_client).

            # Corsia dutching del bus (piano «dutching agganciato», owner
            # 2026-08-23): REQ_PLACE_DUTCHING -> middleware -> CMD_PLACE_DUTCHING
            # -> controller (precheck Roserpina, tavoli, gambe come
            # CMD_QUICK_BET nel choke dell'engine). Il middleware e' agganciato
            # SOLO a questa corsia: le altre hanno gia' il consumer diretto
            # (REQ_QUICK_BET -> engine; REQ_EXECUTE_CASHOUT -> cashout bridge)
            # e un bridge parallelo le eseguirebbe DUE volte. Corsia armata ma
            # dormiente: nessun publisher di REQ_PLACE_DUTCHING esiste ancora.
